...
function endWithFinalize(req: NextRequest, requestId: string, pathname: string, internalPath: string) {
...
export async function middleware(req: NextRequest) {
  ...
  if (staffNeedTotp || pendingFlow) {
    const allow =
      ...
      pathname.startsWith("/api/auth/admin-totp");
    if (allow) {
      return endWithFinalize(req, requestId, pathname, internalPath);
    }
