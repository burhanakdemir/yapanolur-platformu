...
- **`credentials for x` / kimlik reddedildi:** `.env` içindeki `DATABASE_URL` bazen öğretici örnek kalır (`postgresql://x:y@127.0.0.1:5432/...`). Projede bu adres **bağlantı öncesi** reddedilir; `docker-compose.yml` ile uyumlu adres kullanın: `postgresql://ilan:ilan@127.0.0.1:5432/ilan_dev`. (Çok istisnai gerçek kullanıcı adı `x` ise yalnızca geliştirmede `DATABASE_URL_ALLOW_PLACEHOLDER=1`.)
